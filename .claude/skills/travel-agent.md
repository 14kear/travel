...
**Check calendar** (if available) for the travel dates:
- Google Calendar: `gws calendar agenda`, Google Calendar MCP, or `gcal_list_events`
- Apple Calendar (macOS): `icalBuddy eventsFrom:YYYY-MM-DD to:YYYY-MM-DD` or AppleScript
- Other: ask user about meetings on travel dates
